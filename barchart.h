// barchart.h
// TO DO:  this file is responsible for store one entire frame of the animation inside a C - array of Bar objects. 
...
class BarChart {
 ...
 public:
  ...
  void dump(ostream& output) {
    ...
    for (int i = 0; i < size; i++) {
      output << a[i].getName() << " " << a[i].getValue() << " "
  ...
  void graph(ostream& output, map<string, string>& colorMap, int top) {
    ...
    for (int j = 0; j < top; j++) {
      ...
      if (colorMap.empty() || !colorMap.count(a[j].getCategory())) {
        ...
        color =
            colorMap[a[j].getCategory()];  // you should get this from colorMap
      }
      ...
      output << color << barstr << " " << a[j].getName() << " "
             << a[j].getValue() << endl;
    }
